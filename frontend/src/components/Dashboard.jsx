...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        "https://intrusion-detection-system-s1jc.onrender.com/api/feature-importance"
      )
      .then((res) => {
    ...
    axios
      .get(
        "https://intrusion-detection-system-s1jc.onrender.com/api/confusion-matrix"
      )
      .then((res) => {
  ...
  const handleAnalyze = (e) => {
    ...
    featureValues[1] =
      protocolMap[formData.protocol_type] ?? 0;

    featureValues[2] =
      serviceMap[formData.service] ?? 0;

    featureValues[3] =
      flagMap[formData.flag] ?? 0;

    featureValues[4] =
      parseFloat(formData.src_bytes) || 0;

    featureValues[5] =
      parseFloat(formData.dst_bytes) || 0;

    featureValues[6] =
      parseFloat(formData.count) || 0;

    featureValues[7] =
      parseFloat(formData.same_srv_rate) || 0;

    featureValues[8] =
      parseFloat(formData.wrong_fragment) || 0;

    featureValues[9] =
      parseFloat(formData.urgent) || 0;
    ...
    if (
      featureValues[0] > 5 &&
      featureValues[4] > 5000
    ) {
      demoPrediction = "neptune";
    }
      ...
      demoPrediction = "smurf";
    }

    else if (featureValues[8] > 2) {
      demoPrediction = "portsweep";
    }

    else if (featureValues[9] > 0) {
      demoPrediction = "teardrop";
    }

    else if (
      featureValues[5] > 2000 &&
      featureValues[4] < 100
    ) {
      demoPrediction = "back";
    }
  ...
  const pieData = Object.keys(predictionData).map(
    (key) => ({
      name: key,
  ...
  const latestPrediction =
    Object.keys(predictionData)[0] || null;
  ...
  return (
    <div className="dashboard">
      ...
      <p>
        Visualize and analyze network traffic
      ...
      >
        ...
        {latestPrediction && (
          <p
            style={{
              ...
              color: "white",
              backgroundColor:
                ATTACK_COLORS[latestPrediction] ||
                "#888",
              padding: "8px 14px",
              borderRadius: "6px",
        ...
        >
          ...
          {Array.isArray(confusionData) &&
          confusionData.length >= 2 ? (
